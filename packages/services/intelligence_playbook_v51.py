#!/usr/bin/env python3
"""
GFIN Intelligence Playbook & Full Attribution Chain v5.1
Fixed: Better API handling, parallel discovery, integration with server connectors.
"""
...
INTELLIGENCE_PLAYBOOK = {
    "DOMAIN": {
        "what_to_find": [
            ...
            "SSL/TLS certificate details and other domains on same cert",
            "Hosting provider and datacenter location",
            "Page content (scam patterns, wallets, phones, emails, social links, company refs, physical address)",
    ...
    "IP": {
        "what_to_find": ["Geographic location", "ASN", "Hosting provider", "Datacenter address", "Other domains on same IP", "Open ports", "Reverse DNS"],
        "how_to_find": ["IPINFO_IO", "SHODAN", "REVERSE_DNS"],
        "leads_to": ["HOSTING_PROVIDER", "DATACENTER_ADDRESS", "OTHER_DOMAINS", "ASN"],
    },
    "WALLET": {
        "what_to_find": ["All transactions", "Total received/sent", "Current balance", "Downstream wallets", "Exchange deposits", "Cluster analysis"],
    ...
    "COMPANY": {
        "what_to_find": ["Legal name", "Registration number", "Registered address", "Directors", "PSC", "Shareholders", "Filing history", "Status", "Previous names", "Charges"],
        "how_to_find": ["COMPANIES_HOUSE", "OPENCORPORATES"],
        "leads_to": ["PERSON", "ADDRESS", "DOMAIN", "PHONE", "EMAIL"],
    },
    "PERSON": {
        "what_to_find": ["Full legal name", "Aliases", "Directorships", "Addresses from filings", "Public profiles", "Other companies", "Court records"],
    ...
    "ADDRESS": {
        "what_to_find": ["Full physical address", "Type (residential, commercial, virtual office, PO box)", "Other companies at same address", "Coordinates", "Street view", "Is it a virtual office?"],
        "how_to_find": ["COMPANY_REGISTRY", "GOOGLE_MAPS", "OSINT_GEOLOCATION"],
        "leads_to": ["COMPANY", "PERSON"],
    },
    "PHONE": {
        "what_to_find": ["Country and region", "Carrier", "Type (mobile, landline, VoIP)", "Public business references", "Social accounts linked"],
        "how_to_find": ["NUMBER_NORMALIZATION", "PUBLIC_WEB_SEARCH", "SOCIAL_PLATFORM_SEARCH"],
        "leads_to": ["PERSON", "COMPANY", "SOCIAL_ACCOUNT"],
    },
    "EMAIL": {
        "what_to_find": ["Email domain", "Domain registration", "Public references", "Social accounts", "Company associations"],
        "how_to_find": ["EMAIL_FORMAT_ANALYSIS", "DOMAIN_WHOIS", "PUBLIC_WEB_SEARCH"],
        "leads_to": ["DOMAIN", "PERSON", "SOCIAL_ACCOUNT"],
    },
    "SOCIAL_ACCOUNT": {
        "what_to_find": ["Account ID", "Creation date", "Display name and bio", "Profile picture", "Linked website", "Followers", "Post history", "Advertising accounts"],
        "how_to_find": ["PLATFORM_PUBLIC_API", "PUBLIC_PROFILE"],
        "leads_to": ["DOMAIN", "PERSON", "PHONE", "EMAIL"],
    },
    "HOSTING_PROVIDER": {
        "what_to_find": ["Company name", "Registration", "Abuse contact", "Datacenter location", "Terms of service", "Response to abuse reports"],
    ...
    "PAYMENT_PROVIDER": {
        "what_to_find": ["Provider name", "Merchant ID", "Payment flow", "Connected bank accounts (court order)", "Chargeback data (court order)"],
...
TRIGGER_TYPES = {
    ...
    "VICTIM_REPORT": {"description": "Victim reported a scam — domain, phone, email, or wallet", "action": "Run investigation from reported identifier", "priority": "HIGH"},
    "CERTIFICATE_TRANSPARENCY": {"description": "New SSL certificate issued for scam-matching domain", "action": "Investigate domain and all SANs", "priority": "MEDIUM"},
    "SOCIAL_MONITORING": {"description": "Scam content detected in public social media monitoring", "action": "Investigate linked domain/wallet/phone", "priority": "MEDIUM"},
    "PATTERN_MATCH": {"description": "Known scam pattern detected in proactive scan", "action": "Run full investigation from matched entity", "priority": "HIGH"},
    "CAMPAIGN_LINK": {"description": "New entity linked to existing scam campaign via shared evidence", "action": "Investigate new entity + update campaign graph", "priority": "HIGH"},
    "CONTINUOUS_MONITORING": {"description": "Scheduled re-scan of known scam infrastructure detected changes", "action": "Investigate what changed and update evidence", "priority": "MEDIUM"},
    "MANUAL": {"description": "Operator manually initiated investigation", "action": "Run full investigation from provided identifier", "priority": "OPERATOR_SET"},
}


class IntelligencePlaybook:
    """The intelligence engine that knows WHAT to find and HOW — with Subject → Evidence → Physical Address tracing."""
    ...
    def _http_get_json(self, url, timeout=15):
        """HTTP GET that returns parsed JSON or None."""
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "GFIN-Playbook/5.1 (Law Enforcement Research)"})
    ...
    def _http_get_text(self, url, timeout=15):
        """HTTP GET that returns raw text or None."""
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "GFIN-Playbook/5.1 (Law Enforcement Research)"})
            resp = urllib.request.urlopen(req, timeout=timeout, context=self.ssl_ctx)
    ...
    def _dns_lookup(self, domain):
        ...
        try:
            ...
            return list(set(ip[4][0] for ip in ips if ip[4][0].count('.') == 3))
        except:
            return []
    ...
    def investigate(self, subject: dict) -> dict:
        ...
        # Step 3: Execute discovery in parallel
        ...
        if scam_count >= 3 and (loc_count > 0 or people_count > 0 or comp_count > 0):
    ...
    # ============================================================
    # DISCOVERY — parallel execution of all playbook methods
    # ============================================================
    ...
    def _execute_discovery(self, identifier: str, id_type: str) -> list:
        ...
        def run_ipinfo(ip):
            ...
            if data:
                return {"step": self._step_id(), "phase": "IP_GEOLOCATION", "source": "IPINFO_IO",
                        "finding": f"IP {ip} located in {data.get('city', '?')}, {data.get('region', '?')}, {data.get('country', '?')}",
                        "data": {"ip": ip, "city": data.get("city", ""), "region": data.get("region", ""),
                                 "country": data.get("country", ""), "org": data.get("org", ""),
                                 "hostname": data.get("hostname", ""), "loc": data.get("loc", "")},
                        "evidence_id": self._ev_id(), "timestamp": self._ts()}
            return None

        def run_crtsh():
            data = self._http_get_json(f"https://crt.sh/?q={identifier}&output=json", timeout=15)
        ...
        def run_page_content():
            content = self._http_get_text(f"https://{identifier}", timeout=15)
            if not content:
                content = self._http_get_text(f"http://{identifier}", timeout=15)
        ...
        def run_wayback():
            data = self._http_get_json(f"https://web.archive.org/cdx/search/cdx?url={identifier}/*&output=json&limit=10&collapse=urlkey", timeout=15)
        ...
        def run_blockchain(addr):
            ...
            if data:
                return {"step": self._step_id(), "phase": "BLOCKCHAIN_ANALYSIS", "source": "BLOCKCHAIN_INFO",
                        "finding": f"Wallet {addr}: {data.get('n_tx', 0)} tx, {data.get('total_received', 0)/1e8:.4f} BTC received, {data.get('final_balance', 0)/1e8:.4f} BTC balance",
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            ...
            if id_type == "DOMAIN":
                futures[executor.submit(run_rdap)] = "rdap"
                futures[executor.submit(run_dns)] = "dns"
                futures[executor.submit(run_crtsh)] = "crtsh"
                futures[executor.submit(run_page_content)] = "page"
                futures[executor.submit(run_wayback)] = "wayback"
                futures[executor.submit(run_urlscan)] = "urlscan"
            ...
            for future in as_completed(futures):
                try:
                    ...
                    # If DNS resolved IPs, run IP geolocation
                    if futures[future] == "dns" and result and result.get("data", {}).get("ips"):
                        for ip in result["data"]["ips"][:3]:
                            futures[executor.submit(run_ipinfo, ip)] = f"ipinfo_{ip}"

                except:
                    pass

        # If page content was fetched, extract entities
        page_step = results.get("page")
        if page_step and page_step.get("data", {}).get("content"):
            content = page_step["data"]["content"]
            ...
            if wallets:
                steps.append({"step": self._step_id(), "phase": "CRYPTO_WALLET_DISCOVERY", "source": "PAGE_CONTENT_ANALYSIS",
                              "finding": f"Found {len(wallets)} crypto wallet(s): {', '.join(wallets[:3])}",
                              "data": {"wallets": wallets[:5]},
                              "evidence_id": self._ev_id(), "timestamp": self._ts()})
                # Trace each wallet on blockchain
                with ThreadPoolExecutor(max_workers=3) as executor:
                    for w in wallets[:3]:
                        futures = {executor.submit(run_blockchain, w): w}
                        for future in as_completed(futures):
                            try:
            ...
            if phones:
                steps.append({"step": self._step_id(), "phase": "PHONE_DISCOVERY", "source": "PAGE_CONTENT_ANALYSIS",
                              "finding": f"Found {len(phones)} phone(s): {', '.join(phones[:3])}",
            ...
            # Scam pattern detection (using v3 engine if available)
            try:
                from scam_engine_v3 import DeterministicScamEngine
                scam_result = DeterministicScamEngine.analyze(content, identifier)
                if scam_result["summary"]["risk_score"] > 0:
                    steps.append({"step": self._step_id(), "phase": "SCAM_ANALYSIS", "source": "GFIN_ENGINE_V3",
                                  "finding": f"Scam detection: {scam_result['summary']['risk_level']} risk ({scam_result['summary']['risk_score']}), {scam_result['summary']['pattern_count']} patterns, {scam_result['summary']['categories_detected']}",
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                futures = {executor.submit(run_blockchain, identifier): "blockchain"}
                for future in as_completed(futures):
                    try:
                        r = future.result(timeout=20)
                        if r: steps.append(r)
        ...
        for step in discovery_steps:
            ...
            if phase == "ADDRESS_DISCOVERY":
                physical_locations.append({
                    ...
                    "note": "Address found on the website. Must be verified — scammers often use fake or virtual office addresses.",
                })
            ...
            if phase == "CRYPTO_WALLET_DISCOVERY":
                for w in data.get("wallets", []):
                    financial_indicators.append({
                        ...
                        "address": w,
                        "source": "PAGE_CONTENT",
                        "note": "Wallet address found on website. Trace on blockchain to find exchange deposits. Exchange KYC requires court order.",
                    })
    ...
    def _determine_next_steps(self, attribution: dict, accusation_level: str) -> list:
        ...
        if wallets:
            steps.append({"action": "Trace crypto wallets to exchange", "detail": f"Trace {len(wallets)} wallet(s) on blockchain. Identify if funds were sent to a known exchange. Exchange KYC requires court order.", "priority": "HIGH", "legal_authority_needed": "Court order to exchange for KYC records"})
        ...
        for loc in locs:
            if loc["type"] == "HOSTING_LOCATION":
                ...
                steps.append({"action": "Verify page address", "detail": f"Address on website: {loc.get('address', '?')}. May be fake. Verify via independent sources.", "priority": "LOW"})
    ...
    def _generate_report(self, inv: dict) -> str:
        """Generate Subject → Evidence → Physical Address narrative report."""
        lines = []
        subj = inv["subject"]
        lines.append("=" * 70)
        ...
        if inv["financial_indicators"]:
            ...
            for fi in inv["financial_indicators"]:
                ...
                if fi.get("total_received_btc") is not None:
                    lines.append(f"  Received: {fi['total_received_btc']} BTC, Sent: {fi.get('total_sent_btc', 0)}, Balance: {fi.get('final_balance_btc', 0)}, TXs: {fi.get('transaction_count', 0)}")
